...
O projeto atende a requisitos de sistemas distribuídos:
- **Identificação de Sessão**: O controle de acesso e a identificação intrínseca aos pacotes (tanto em texto quanto em vídeo/áudio) é feito informando `--user` na hora de rodar o cliente.
...
1. **Registry (`registry.py`)**: Atua como o "Service Discovery" (Descoberta de Serviço). Ele mantém uma lista dos brokers que estão ativos, recebe *heartbeats* (monitoramento da saúde dos brokers) e informa aos clientes em qual broker eles devem se conectar inicialmente.
