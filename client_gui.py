...
class VideoConferenceClientGUI:
    def __init__(self, registry_endpoint, user_id, room="ROOM_A"):
        ...
        self.root = tk.Tk()
        self.root.title(f"VideoConf - {self.user_id} ({self.room})")
    ...
    def start(self):
        self.print_to_chat(f"[*] Conectando ao sistema distribuído...")
        self.root.update() 
        
        if not self.discover_and_connect():
            self.print_to_chat("[!] Falha Crítica de conexão.")
            return
            
        self.print_to_chat(f"[*] Conectado com sucesso à sala '{self.room}'!\n-------------------")
        ...
        # O Loop Limpo e Seguro: Puxa vídeos e textos das filas simultaneamente
        while self.running:
            try:
                # 1. Puxa Vídeos
                try:
                ...
                # 2. Puxa Textos da Rede (Garante que nunca sejam ignorados)
                try:
                ...
                self.root.update()
                time.sleep(0.01) # Permite que o processador "respire" e evite lag
        ...
        while self.running:
            try:
                ...
                if sub_sock in socks:
                    # DRENO DE BUFFER (NOBLOCK): Puxa todas as mensagens represadas na mesma fração de segundo
                    while True:
                        try:
                            ...
                            if len(parts) >= 2:
                                ...
                                if msg_type == "TEXT":
                                    ...
                                    if data.get("user_id") != self.user_id:
                                        ...
                                        self.text_queue.put(msg) # Envia para a fila segura
                            ...
                            break # Fila do ZeroMQ esvaziada com sucesso, volta a aguardar
            except Exception:
                time.sleep(0.1)
    ...
    def send_text(self, text):
        ...
        while retries > 0 and self.running:
            ...
            retries -= 1
            if retries > 0:
                self.text_queue.put(f"[!] Falha na rede. Tentativa {3 - retries}/3...")
        
        if self.running:
            self.text_queue.put("⚠️ Broker off-line! Migrando rota...")
...
if __name__ == "__main__":
    ...
    client = VideoConferenceClientGUI(args.registry, args.user, args.room)
    client.start()